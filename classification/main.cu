...
void prediction(Ctxt *c, Ctxt *c_const, Ctxt **o, Ctxt **v);
...
int main(int argc, char *argv[]) {
    ...
    // Allcate voting result array.
    Ctxt **v = new Ctxt*[n_trees];
    for (int n = 0; n < n_trees; ++n)
    {
        v[n] = new Ctxt[n_class];
        for (int m = 0; m < n_class; ++m)
            Encrypt(v[n][m], p_const[0], pri_key);
    }

    // Prediction.
    std::cout << ":: Prediction" << std::endl;
    prediction(c, c_const, o, v);

    // Get result.
    std::cout << ":: Get result" << std::endl;
    int result = get_result(v, pri_key);
    ...
    for (int n = 0; n < n_trees; ++n)
        delete[] o[n];
    delete[] p, p_const, c, c_const, o, v;
